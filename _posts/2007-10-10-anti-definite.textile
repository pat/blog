...
I love it when sorting ignores prefixes of 'The' or 'A' - definite or indefinite articles, if you want the correct terms. To get this happening within a SQL @SELECT@ request is a bit tricky, but inspired by "someone else's solution":http://www.arraystudio.com/as-workshop/how-to-sort-mysql-results-ignoring-definite-and-indefinite-articles-the-a-an.html (Ok, so I pretty much copied their code), I've created the following function to make it a lot easier:

<pre><code>CREATE FUNCTION ANTIDEFINITE(field VARCHAR(1024))
...
<pre><code>SELECT *
FROM posts
ORDER BY ANTIDEFINITE(subject);</code></pre>

Or, via Rails:

<pre><code>Post.find :all, :order => 'ANTIDEFINITE(subject)'</code></pre>

...
If you're offering a patch to Thinking Sphinx, the best way to do this is to fork "the GitHub project":http://github.com/freelancing-god/thinking-sphinx, patch it, and then send me a Pull Request. This last step is important - while I may be following your fork on GitHub, the request means an email ends up in my inbox, so I won't forget about your changes.
...
<h3 id="cucumber">Running Cucumber Features</h3>

Thinking Sphinx's Cucumber features require both a database, and port 3312 to run Sphinx on. The latter should take care of itself provided you're not using that port already. The former is managed by @features/support/database.yml@ (with an example file at @features/support/database.example.yml@).
...
<h3 id="gems">Writing Third-Party Gems</h3>
...
I recommend looking at the "Delayed Delta":http://github.com/freelancing-god/ts-delayed-delta/ library for inspiration.
